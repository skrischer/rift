...
mod tests {
    ...
    // ── ConnectionScreen (GPUI) ──────────────────────────────────────────

    fn sample_defaults() -> ConnectDefaults {
        ConnectDefaults {
    ...
    /// Host a fresh [`ConnectionScreen`] in a `Root`-wrapped test window,
    /// mirroring `command_palette`'s test harness of the same shape.
    fn screen_window(
        cx: &mut TestAppContext,
        recents: Vec<RecentConnection>,
    ) -> (Entity<ConnectionScreen>, WindowHandle<Root>) {
        let mut screen: Option<Entity<ConnectionScreen>> = None;
        let window = cx.update(|cx| {
            gpui_component::init(cx);
            cx.open_window(WindowOptions::default(), |window, cx| {
                let view = cx
                    .new(|cx| ConnectionScreen::new(&sample_defaults(), recents, None, window, cx));
        ...
        (
            screen.expect("constructed inside the window callback"),
            window,
        )
    }

    fn sample_recent(host: &str) -> RecentConnection {
        RecentConnection {
            host: host.to_string(),
            user: "alice".to_string(),
            port: 2222,
            key: "/home/alice/.ssh/id_ed25519".to_string(),
            session: "work".to_string(),
            last_connected_unix_secs: 123,
        }
    }

    #[gpui::test]
    fn test_new_prefills_every_input_from_defaults(cx: &mut TestAppContext) {
        let (screen, _window) = screen_window(cx, Vec::new());

        cx.update(|cx| {
            let screen = screen.read(cx);
            assert_eq!(screen.host_input.read(cx).value().as_ref(), "100.64.0.1");
            assert_eq!(screen.user_input.read(cx).value().as_ref(), "developer");
            assert_eq!(screen.port_input.read(cx).value().as_ref(), "22");
            assert_eq!(
                screen.key_input.read(cx).value().as_ref(),
                "/home/developer/.ssh/id_ed25519"
            );
            assert_eq!(screen.session_input.read(cx).value().as_ref(), "rift");
        });
    }

    #[gpui::test]
    fn test_valid_input_emits_connect_request_with_the_typed_values(cx: &mut TestAppContext) {
        let (screen, window) = screen_window(cx, Vec::new());
        let emitted: Rc<RefCell<Vec<ConnectRequest>>> = Rc::new(RefCell::new(Vec::new()));

        let _subscription = cx.update(|cx| {
            let emitted = emitted.clone();
            cx.subscribe(
                &screen,
                move |_screen, event: &ConnectionScreenEvent, _cx| {
                    let ConnectionScreenEvent::Connect(request) = event;
                    emitted.borrow_mut().push(request.clone());
                },
            )
        });
        ...
        assert_eq!(events[0].host, "100.64.0.1");
        assert_eq!(events[0].user, "developer");
        assert_eq!(events[0].port, 22);
        assert_eq!(events[0].session, "rift");

        cx.update(|cx| {
            assert!(screen.read(cx).error.is_none());
        });
    }

    #[gpui::test]
    fn test_empty_host_sets_error_and_does_not_emit(cx: &mut TestAppContext) {
        let (screen, window) = screen_window(cx, Vec::new());
        ...
        let _subscription = cx.update(|cx| {
            ...
            cx.subscribe(
                &screen,
                move |_screen, _event: &ConnectionScreenEvent, _cx| {
        ...
        cx.update(|cx| {
            assert!(screen.read(cx).error.is_some());
        });
    }

    #[gpui::test]
    fn test_invalid_port_sets_error_and_does_not_emit(cx: &mut TestAppContext) {
        let (screen, window) = screen_window(cx, Vec::new());
        let emitted = Rc::new(RefCell::new(0usize));
        ...
        cx.update_window(window.into(), |_, window, cx| {
            screen.update(cx, |screen, cx| {
                screen
                    .port_input
                    .update(cx, |input, cx| input.set_value("not-a-port", window, cx));
                screen.try_connect(cx);
            });
        })
        .unwrap();
    ...
    #[gpui::test]
    fn test_blank_session_defaults_to_rift(cx: &mut TestAppContext) {
        let (screen, window) = screen_window(cx, Vec::new());
        let emitted: Rc<RefCell<Vec<ConnectRequest>>> = Rc::new(RefCell::new(Vec::new()));
        ...
        assert_eq!(emitted.borrow()[0].session, "rift");
    }

    #[gpui::test]
    fn test_clicking_a_recent_row_prefills_and_emits_connect_with_its_values(
        cx: &mut TestAppContext,
    ) {
        let recent = sample_recent("10.0.0.5");
        let (screen, window) = screen_window(cx, vec![recent]);
        let emitted: Rc<RefCell<Vec<ConnectRequest>>> = Rc::new(RefCell::new(Vec::new()));
        ...
        cx.update_window(window.into(), |_, window, cx| {
            screen.update(cx, |screen, cx| {
                screen.connect_from_recent(0, window, cx);
            });
        })
        .unwrap();
        ...
        assert_eq!(events[0].port, 2222);
        assert_eq!(events[0].session, "work");

        cx.update(|cx| {
            let screen = screen.read(cx);
            assert_eq!(screen.host_input.read(cx).value().as_ref(), "10.0.0.5");
            assert_eq!(screen.user_input.read(cx).value().as_ref(), "alice");
        });
    }

    #[gpui::test]
    fn test_clicking_a_stale_recent_index_is_a_no_op(cx: &mut TestAppContext) {
        let (screen, window) = screen_window(cx, Vec::new());
        let emitted = Rc::new(RefCell::new(0usize));
    ...
    #[gpui::test]
    fn test_focus_handle_targets_the_host_input(cx: &mut TestAppContext) {
        let (screen, window) = screen_window(cx, Vec::new());
        ...
        cx.update_window(window.into(), |_, window, cx| {
            ...
            assert_eq!(
                window.focused(cx),
                Some(screen.read(cx).host_input.focus_handle(cx))
            );
        })
        .unwrap();
    }
