...
///
/// The daemon is always spawned without `--root`: it starts root-less and
/// derives its watched root from the session it attaches (`@root`, or the
/// tmux `session_path` for sessions created outside rift).
