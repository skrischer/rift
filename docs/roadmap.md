...
| 25 | Explorer design parity — header actions, git letter lane, diagnostic dots + rollup, empty states | [archive/spec-explorer-parity.md](archive/spec-explorer-parity.md) | [Phase 250](https://github.com/skrischer/rift/milestone/39) |
| 26 | Settings shell + theme unification — full settings page, theme-driven terminal palette, hardcoded-hex cleanup | [archive/spec-settings-theme.md](archive/spec-settings-theme.md) | [Phase 260](https://github.com/skrischer/rift/milestone/40) |
...
| 34 | Session start-directory — new panes / windows / sessions spawn in the session's project root (`-c` on new-session / new-window / split-window + `attach-session -c` to re-root a pre-existing session), replacing the `$HOME` landing; single-root (no per-session dynamize yet) | [archive/spec-session-start-directory.md](archive/spec-session-start-directory.md) | [Phase 340](https://github.com/skrischer/rift/milestone/52) |
| 35 | Per-session project root — the daemon watched root follows the active session; the root is coupled to the tmux session via a session-scoped `@root` user option, resolved daemon-side on attach, superseding the single baked `RIFT_PROJECT_ROOT`; a session switch re-roots the reactive layer (file tree / git / LSP). session = project | [archive/spec-per-session-project-root.md](archive/spec-per-session-project-root.md) | [Phase 350](https://github.com/skrischer/rift/milestone/53) |
| 36 | New-session remote root picker — creating a session picks its project root by browsing the remote filesystem (a daemon-side directory listing), binding session = project at creation: the name defaults to the folder basename and the root is written to `@root` (phase 35). Supersedes the zero-sessions picker screen — with no sessions, connecting opens the root picker directly | [archive/spec-session-root-picker.md](archive/spec-session-root-picker.md) | [Phase 360](https://github.com/skrischer/rift/milestone/55) |
...
| 48 | Multi-language LSP — populate the daemon's `ServerSpec` registry beyond the single rust-analyzer row (pyright, typescript-language-server, gopls, clangd), selected by the existing extension-based `DocumentSelector`; servers are consumed from the remote `$PATH`, never installed. The registry + filetype routing already exist (Phase 3.4); this fills the table | [archive/spec-lsp-servers.md](archive/spec-lsp-servers.md) | [Phase 480](https://github.com/skrischer/rift/milestone/69) |
| 49 | Terminal scrollback scrollbar — a visible, auto-hiding vertical scrollbar over the terminal history tied to scrollback position, using gpui-component's `Scrollbar` (the widget the pickers adopted in #804) | [archive/spec-terminal-scrollbar.md](archive/spec-terminal-scrollbar.md) | [Phase 490](https://github.com/skrischer/rift/milestone/70) |
| 50 | Terminal output timestamps (on-demand) — stamp PTY-byte arrival time per scrollback line and surface it on demand (hover-gutter / idle-marker / toggle), agent-agnostic (no output parsing). Closes the gap iTerm2 has under tmux (one timestamp per window) by stamping per pane at the control-mode `%output` boundary | [archive/spec-terminal-timestamps.md](archive/spec-terminal-timestamps.md) | [Phase 500](https://github.com/skrischer/rift/milestone/75) |
| 51 | Markdown preview — a source/preview toggle for `.md` files rendering via gpui-component's `Markdown` element (read-only first; no live split-edit) | [archive/spec-markdown-preview.md](archive/spec-markdown-preview.md) | [Phase 510](https://github.com/skrischer/rift/milestone/71) |
| 52 | WSL transport — a first-class WSL connection type alongside SSH (Zed's SSH/WSL/Docker `RemoteConnection` split), with a connection-kind chooser on the connect card; distinct from the `RIFT_REMOTE_EXEC_WRAPPER` one-hop-deeper wrapper | [archive/spec-wsl-transport.md](archive/spec-wsl-transport.md) | [Phase 520](https://github.com/skrischer/rift/milestone/73) |
| 53 | Agent activity: working vs idle signal — discriminate an agent pane actively working from one idle-awaiting-input, which today's `is_shell` foreground-command signal cannot (an agent pane reads permanently Busy). Adds a per-pane `/proc` `pane_pid` CPU roll-up (the Phase-45 method) and/or PTY output-idle timing; strictly agent-agnostic. Depends on 45 | [archive/spec-agent-activity.md](archive/spec-agent-activity.md) | [Phase 530](https://github.com/skrischer/rift/milestone/78) |
| 54 | Global UI font size — a real UI font-size / zoom control applied to editor, explorer, and chrome (not only the terminal PTY grid, which is all today's mislabelled "Font size" slider touches); folds the explorer-font-too-large papercut | [archive/spec-ui-font-size.md](archive/spec-ui-font-size.md) | [Phase 540](https://github.com/skrischer/rift/milestone/72) |
| 55 | Real editor minimap — replace the 32px marks-strip (line-length bars, click-to-jump only) with a scaled code miniature plus a viewport indicator and drag-to-scroll | [archive/spec-editor-minimap.md](archive/spec-editor-minimap.md) | [Phase 550](https://github.com/skrischer/rift/milestone/76) |
| 56 | Independent Explorer/Editor visibility — split the combined `Area::ExplorerEditor` rail item into two independently-toggleable areas (two nav items, filled-region icon indicating the target); reverses Phase 39's deliberate Explorer+Editor unification | [spec-explorer-editor-split.md](archive/spec-explorer-editor-split.md) | [Phase 560](https://github.com/skrischer/rift/milestone/77) |
...
`return_to_connection_screen` unconditionally (`crates/app/src/main.rs:1128` / `:1466`
/ `:1693`), so a session kill wrongly tears down the live SSH/daemon connection.
Target: on the attached session ending while the connection is alive, re-query the
live session list and route — **≥1 session remaining → the session picker** (always,
even for exactly one; no auto-attach); **0 remaining → the zero-sessions root picker**
(the create flow, superseding the empty-state per phase 36). The **connection screen
is entered only on a real SSH/transport loss**, never on a session kill. Reuses the
existing `PickerOutcome::ShowPicker(sessions)` empty-vs-non-empty routing
(`main.rs:1228-1249` → `show_root_picker` / `show_session_picker`) and the recovery
engine's re-Attach — but with a **live daemon client**. Motivating symptom: issue #793.

The crux: the app must support **"connected, no active session"** as a first-class
**mid-session** state and re-enter the pre-cockpit `ScreenState::Picker` /
`ScreenState::RootPicker` (today pre-cockpit only, `main.rs:1159`) without dropping the
connection — i.e. "session ended" must stop meaning "disconnected".

Foundation impact: `docs/architecture.md` — the phase-20 "Connection robustness
contract" gains the **mid-session sessionless state** and the **session-end vs.
