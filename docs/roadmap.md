...
| 18 | Window-tab pane-activity indicators — active-pane count + aggregate state (busy / idle / attention) per window, derived agent-agnostically from tmux `monitor-activity`/`-silence`/`-bell` alerts over control mode | — | — |
