...
- 2026-08-06: Second adversarial pass on PR #976 found a real, distinct residual **and** a false claim in the two entries above. The false claim: "only undercounts an auto-wrapped, non-newline-terminated line" — wrong; verified with a newline-*terminated* wrapped line that still drifted. The real residual: once fully saturated, `window_growth` is *permanently* 0 (both getters flat by definition of "saturated"), so growth there is *always* just the `\n` count, full stop — an auto-wrapped line adds one grid row per wrap with no `\n` byte of its own, regardless of whether the line itself ends with `\n`, so every such wrap is silently undercounted. This is **assessed and accepted as a spike limitation, not fixed here**: a robust wrap-birth counter would have to simulate the cursor against arbitrary escape sequences (colors, cursor moves, TUI redraws) — i.e. reimplement a VTE — which is out of scope for this module and would be least reliable on exactly the escape-heavy agent output this tool targets. Net effect, characterized and pinned by `test_wrapped_line_at_saturation_drifts_by_wrap_count_known_limitation`: pre-saturation growth and any saturated batch of newline-terminated, non-wrapping lines remain exact; **post-saturation line-id (hence timestamp) mapping drifts proportional to wrapped-line volume**. Handed to #933: the timestamp backend must mitigate (e.g. periodic re-anchor / drift-tolerant keying) or explicitly accept bounded drift for the on-demand hover surface — this is not near-exact and must not be presented as such.
