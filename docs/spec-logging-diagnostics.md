...
- [ ] **The daemon logs through `tracing`**: `eprintln!` is gone from the daemon binary; stderr carries an `EnvFilter`-governed fmt subscriber, and a daemon-managed size-rotated file sink (same rotation logic) replaces unbounded `>>` append growth on the remote — while **stdout stays frame-only** in `--connect` relay mode (the #60 framing invariant is untouched and test-guarded).
...
- **Daemon wiring** (`crates/daemon`): `eprintln!` → `tracing` with a stderr fmt subscriber + the shared filter chain; a `--log-file`/env-configured rotated file sink for the long-running `--serve-uds` server (default: the path the launch line redirects to today). The launch line's `>> log 2>&1` redirect stays as the pre-init/panic backstop — it catches what no subscriber can.
- **Dev-loop fit**: `just dev-watch` / `dev-windows-watch` console behavior is unchanged by default (TTY → console); the stable shortcut launch is unchanged except rotation (no TTY → file).
...
- **Per-run PID-keyed log files with age pruning** (the wezterm model) — noted in the survey as the fallback model if the daemon's single rotated file proves wrong; not built now.
...
- **TTY detection with env override**: the forced cases (windowed exe that should log to an attached console, redirected dev run that should keep the console sink) get one `RIFT_*` env override, mirroring Zed's force flag — not a second feature gate.
...
| **Daemon: stderr + daemon-managed rotated file; launch redirect stays as pre-init/panic backstop; stdout untouched** | Constraint-determined: the #60 framing invariant makes stdout sacred in relay mode; the current `>> log` shell append is unbounded for a long-running daemon; the daemon managing its own rotated sink reuses the shared writer (unified setup — the track's name). | 2026-06-12 |
...
- [ ] Milestone QA (dev channel): a dev-watch session shows readable, filtered console logs; the stable channel's log pair is inspectable after a silent-death scenario
...
| The daemon's file sink and the launch-line `>>` redirect write the same path | Default the `--log-file` sink to its own path distinct from the redirect target (e.g. `.log` vs `-stderr.log` naming pinned in the issue) — the redirect is the pre-init backstop, not the primary sink; both rotate/bound or stay tiny. |
