...
- 2026-07-02: Issue #231 (daemon wiring) implemented. The distinct default file-sink path for `--serve-uds` is the socket path with `.log` appended (`<socket>.sock.log`), mirroring the existing `pidfile_path` precedent (`<socket>.sock.pid`) — never colliding with the launch line's `<binary>.log` redirect target since the socket path itself is `<binary>.sock`. A `--serve-uds`-only `--log-file <path>` flag and a `RIFT_DAEMON_LOG_FILE` env var (flag wins) override that default, matching the `RIFT_DAEMON_BINARY`/`RIFT_DAEMON_REMOTE_DIR` naming already used for other daemon-specific overrides. Review caught that giving `--connect` a socket-keyed default of its own would collide with its `--serve-uds` daemon's file: every real launch (`crates/ssh/src/launch.rs`) drives both against the identical socket path, and `SizedWriter`'s rotation is not safe across independent processes sharing one file — exactly the "one writer per file pair" constraint this spec calls out. Fixed by installing no subscriber at all for `--connect`, mirroring `--ping`'s existing rationale (`relay`/`connect_relay` have no call sites worth logging today); this is spec-compliant since the stdout-frame-only constraint only restricts what a subscriber may target, it does not mandate one exist. The (production-unused) default stdio mode keeps its own unrelated, non-colliding default (`rift-daemon.log`, no socket path to key off). `install_panic_hook()` runs unconditionally before mode dispatch, in every mode.
