...
- 2026-07-02: Issue #231 (daemon wiring) implemented. The distinct default file-sink path is the socket path with `.log` appended (`<socket>.sock.log`), mirroring the existing `pidfile_path` precedent (`<socket>.sock.pid`) — never colliding with the launch line's `<binary>.log` redirect target since the socket path itself is `<binary>.sock`. A `--serve-uds`-only `--log-file <path>` flag and a `RIFT_DAEMON_LOG_FILE` env var (flag wins) override that default, matching the `RIFT_DAEMON_BINARY`/`RIFT_DAEMON_REMOTE_DIR` naming already used for other daemon-specific overrides. `--connect` and the (production-unused) default stdio mode apply the same TTY rule for parity with the stdout-frame-only constraint, deriving their own default file path the same way; `--ping` installs no subscriber at all (no call sites on that path, and it runs on every reconnect probe). `install_panic_hook()` runs unconditionally before mode dispatch, in every mode.
