...
- 2026-06-15: Daemon navigation routing implemented (#195). `OwnedNavRequester` introduced in `rift-lsp` — owns a cloned `ServerSocket` (`Clone + Send + 'static`) so it can be moved into `tokio::spawn` without capturing the server registry. Drop-stale discipline: `LspWorker` tracks `latest_hover_id / latest_definition_id / latest_references_id`; `forward_nav_response` discards responses whose id is not the latest for that operation type. `Registry::first_capable_for_path` added — iterates selector matches and by-language index synchronously, no registry lock contention (the registry is owned by the worker task). Disk read (`read_text_from_disk`) is wrapped in `tokio::task::spawn_blocking` to respect the "async for I/O, spawn_blocking for CPU/blocking work" rule. Symmetric channel cleanup: when `nav_responses` closes (worker exited), both `nav_responses = None` and `core.nav_requests = None` are set so subsequent forward attempts are silently dropped.
- 2026-06-15: Out-of-root **read** carve-out implemented (#195 reopened; #301). Corrects the framing in the #195-merge note above: that PR shipped only the **save refusal** and mistakenly treated "out-of-root `SaveFile` is refused" as the whole carve-out — it even asserted out-of-root **reads** are *refused*, the opposite of the spec decision (2026-06-12: reads **served** read-only, saves refused). The carve-out is fundamentally about *serving* the read so the editor can open a stdlib/dependency definition target. Implementation: `buffer::resolve_read` splits the read path — a **relative** path stays root-confined (`..`/symlink escapes refused via the unchanged `resolve`); an **absolute** path is served **read-only** as the out-of-root carve-out, returned as-is with no confinement check. `read_file` routes through `resolve_read`; `write_file` is untouched and still routes through `resolve`, which refuses every absolute path — so the carve-out can never widen into a write outside root. No protocol change: out-of-root targets reuse `OpenFile`/`FileContent`, and the app marks the buffer read-only off `NavLocation.out_of_root` (already a `bool` on the wire). Read-only enforcement is client-side; the daemon serves the bytes on a single-user remote (the accepted threat model). The mistaken read-refusal test was flipped to assert content is served (`test_out_of_root_read_file_absolute_path_is_served_read_only`), a direct served-read-but-write-refused test was added, and the old `test_read_rejects_absolute_path` became `test_write_rejects_absolute_path` (the absolute-refusal guarantee now lives on the write path).
