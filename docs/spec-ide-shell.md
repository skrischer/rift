...
- 2026-07-03: Issue #325 (dock interaction — resize, toggle, zoom, focus) implementation. Verified against the pinned `gpui-component` rev (`9ad30e63`, `crates/ui/src/dock/{dock,mod,tab_panel,stack_panel}.rs`) that drag-resize (`Dock`/`StackPanel`'s native `resize_handle`/`ResizablePanelGroup`), dock toggle (`DockArea::toggle_dock`, rendered automatically by `TabPanel::render_dock_toggle_button`), zoom (`PanelEvent::ZoomIn`/`ZoomOut` -> `DockArea::set_zoomed_in`/`set_zoomed_out`, rendered by `TabPanel::render_toolbar`), and focus-follows-active-panel (`TabPanel::focus_active_panel`) are all fully wired by the vendored dock with zero rift code — none of `FileTree`/`EditorView`/`TerminalPanel` need to override any `Panel` default. No production code added; per "reuse, don't rebuild" this issue's payload is regression tests for the headlessly-testable slice (dock toggle via `DockArea::toggle_dock`/`is_dock_open`, the zoomable-by-default invariant, and the workspace-focus-delegates-to-terminal invariant) plus the grep verification (no agent detection; `rift-terminal`'s `Cargo.toml`/source has no `dock`/`Panel` usage). Drag-resize and the visual feel of toggle/zoom stay deferred to the milestone QA gate per this spec's existing "headless-testable where it can be" constraint — mouse-driven interaction has no headless surface to assert on.
