...
- [ ] State survives an unclean exit (kill, crash, reboot): persistence is change-driven (debounced) with atomic writes — not only on clean shutdown.
...
- **A small window-state store** (`crates/app`): serde-JSON schema (bounds, maximized, font scale, schema version), tolerant load (defaults on missing/corrupt/unknown), debounced change-driven save with atomic temp+rename writes.
...
- **Atomic writes** (temp + rename), mirroring the established buffer-service pattern — a crash mid-save never corrupts the state file; a corrupt file degrades to defaults anyway (belt and suspenders).
- **Debounced saves** on change (the Zed `SERIALIZATION_THROTTLE_TIME` precedent, `prior-art.md` Category 1) — no save per pixel of a drag; cadence pinned in the issue.
...
| **Change-driven debounced saves, not save-on-exit** | Constraint-determined: the stable channel dies by kill/reboot more often than by clean exit (detached process, `promote` relaunch); save-on-exit would lose exactly the sessions this spec exists for. Zed precedent. | 2026-06-12 |
...
- [ ] Restart restores position, size, maximized state, and font scale (manual QA: close/reopen, `just promote` relaunch, reboot-then-shortcut)
