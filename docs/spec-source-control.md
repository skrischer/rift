...
- **Daemon diff capability** (`crates/explorer` + `crates/daemon` + `crates/protocol`): compute a per-file diff of the working-tree/index content against the HEAD blob using **gix's blob diff** (enable gix's `blob` feature — `gix-imara-diff` is already in the dependency tree; no new crate). New protocol messages: a `RequestDiff { path }` `ClientMessage` and a `FileDiff { path, ... }` `DaemonMessage` carrying a structured diff (hunks with old/new line ranges + line content and add/remove/context tags), plus binary/too-large sentinels. Computed on request (like `OpenFile`), not pushed.
...
- **Diff computed with gix (`blob` feature), not a new diff crate**: `gix-imara-diff` is already transitively present; enabling gix's `blob` feature exposes blob diffing. `similar` (MIT/Apache) is the named fallback **only** if gix's blob-diff API proves insufficient at the pinned `gix 0.84` — flagged so either is spec-sanctioned, but gix-first (no new top-level crate) is the intent.
...
- **Binary and oversized handling is explicit**: the daemon detects binary (non-UTF-8 / gix binary heuristic) and a size ceiling, returning a sentinel the client renders as a placeholder — never a partial/garbled diff.
...
- **Depends on Phase 10 (dock shell)**: the panel docks into a Phase 10 zone (right or a secondary side dock — exact placement an implementation detail). Milestone depends on Phase 100.
...
| PR size: protocol + daemon + panel + diff view | Decompose: (1) protocol diff types + gix `blob` diff compute + daemon handler; (2) source-control panel (file list) into the dock; (3) virtualized diff view + binary/large handling; (4) refresh/live-tracking wiring. ~400-line issues. |
