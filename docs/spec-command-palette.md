...
- **Shell command actions** *(OPEN — resolved at the spec-acceptance gate; recommended: in scope)*: Phase 10 delivers dock/panel toggling via the `DockArea`'s built-in mouse controls, **not** as dispatchable `#[action]` types (see `spec-ide-shell.md`), so those commands do not exist yet. Recommended: **Phase 16 defines them** — a small set of `#[action(namespace = rift)]` types (toggle explorer / problems / source-control panels, focus terminal, zoom active panel) wired to the Phase 10 `DockArea` entity — since they are the palette's most valuable content and no other phase owns them. The alternative is a thinner v1 palette over the existing editor/nav actions only.
...
| **Shell command actions (dock/panel toggle, focus, zoom) in scope** | **OPEN — resolved at the spec-acceptance gate.** Phase 10 delivers toggling via mouse controls, not `#[action]` types, so those commands do not exist. Recommended: Phase 16 defines them (wired to the `DockArea`) — they are the palette's most valuable content and no other phase owns them. Alternative: a thinner v1 over editor/nav actions only. | OPEN |
