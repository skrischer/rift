...
- 2026-07-03: Implementation of #212 (command interception + refresh). The constraint's assumption that "the Phase 6 protocol carries [command-response] anyway" turned out to require net-new work, not reuse: #210/#211 built only the parsing/dispatch layer in `crates/terminal`, and the daemon's `TmuxCommand` seam was fire-and-forget with no reply plumbed through for arbitrary commands (`capture-pane`/layout each had their own hand-built request/response pair; there was no generic one). Added a fourth protocol request/response pair, `QueryKeyTable` → `KeyTableReply` (`crates/protocol`), tracked in the daemon's `Attach` the same way as its existing `layout_query`/`captures` (`crates/daemon/src/terminal.rs`), issued unprompted on attach/reconnect (mirroring the layout query) and on-demand for the explicit-trigger and binding-mutating-dispatch refreshes. Scoped to the daemon terminal path only, not the legacy direct-tmux path (`run_legacy_terminal` in `crates/app`) — consistent with the constraint's own framing that Phase 7's dispatch is "the daemon one from day one"; a refresh request in legacy mode is a harmless no-op once its channel's receiver is dropped. The command-interception taxonomy (`classify_command`/`DispatchDecision`) and `confirm-before` parsing landed in `crates/terminal/src/keytable.rs`; hints use gpui-component's `Notification` (auto-dismissing toast) and the confirm dialog uses its `AlertDialog` — both previously unused in this codebase. The explicit refresh trigger is a small statusbar button ("refresh keys") next to the pending-prefix indicator.
