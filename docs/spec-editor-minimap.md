...
| **OPEN — resolved at the spec-acceptance gate:** how far v1 goes given the pin has no scroll-offset setter and no token accessor — (A) stay in-API: indent+length+diagnostic-shaded blocks, drag via `set_cursor_position` (the caret follows the drag); or (B) commit to a longbridge gpui-component fork/upstream adding a scroll-offset setter (clean caret-free drag) and optionally a token-span accessor (token-colored miniature) | A ships now with no dependency fork but a caret-follow drag wart and no token color; B is higher fidelity but commits rift to maintaining a gpui-component fork or an upstream dependency. Neither the constitution nor prior art settles the fork-vs-in-API tradeoff | 2026-08-01 |
