...
- [ ] **Three states**, precedence **attention > busy > free**:
  - **busy** — a foreground command is running in the pane (OSC-133 lifecycle `Executing`); stays busy while the process is silent (an agent thinking mid-turn is still busy, not free).
  - **free** — the pane is at a shell prompt with no command running (OSC-133 `PromptShown`/`Idle`).
  - **attention (waiting)** — the pane rang the terminal bell (BEL) and the user has not yet acknowledged it; overlays busy/free and dominates the aggregate.
...
| **Output-recency fallback for panes without OSC-133** | Robustness: a pane whose shell emits no OSC-133 markers would otherwise always read "free" while a process runs; a "busy if output within the idle window" fallback keeps the count meaningful. Precise OSC-133 wins when present. | 2026-07-02 |
...
| **Attention (waiting) clears on acknowledgement = window selection** | The bell means "look at me"; once you switch to the window you have looked. Simple, deterministic, no timer. | 2026-07-02 |
| **OPEN — is the terminal *bell* the accepted agnostic "waiting" signal (best-effort, agent-dependent), or does v1 ship free/busy only?** | resolved at the spec-acceptance gate | — |
...
| The terminal bell is unreliable for "waiting" — an agent may not ring it | This is the genuinely-open gate decision. Framed honestly as best-effort/agent-dependent; the fallback if rejected is a 2-state (free/busy) indicator. Claude Code does ring the bell on permission prompts and turn end, but agents are black boxes. |
