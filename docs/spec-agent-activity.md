...
- 2026-08-06: Issue #952 (shared `PaneActivity` model + render-thread issue) implemented, without a signal source — behavior-preserving plumbing. `PaneActivity` gained a new `BusyIdle` variant (sibling to `Busy`, not associated data on it) so every existing `PaneActivity::Busy` match/test keeps compiling unchanged; `ActivityTracker` gained a `WorkState` (`Working`/`Idle`, default `Working`) folded into `Busy` only when the structural `is_shell` gate is already busy, so a shell pane is unaffected and, with no caller of the new `PaneView::set_work_state` seam yet, every pane keeps reading today's plain `Busy`. Aggregate precedence: `attention > busy(working) > busy(idle) > free` — a window with any actively-working pane reads working even alongside idling panes, since "something is actively happening" reads as the more salient at-a-glance state; idle-awaiting-input still outranks free. Render treatment: `TabStateSlot::BusyIdle` and the status-line `window_chip` reuse the busy dot's exact shape/size, distinguished only by color (`warning` vs `success`) — the shape distinction stays reserved for busy-vs-attention. This precedence and color choice is an implementation default for #953 to revisit, not a re-litigated spec decision.
- 2026-08-06 (#953 implementation, activation-model conflict discovered and resolved): implementing the CPU classifier surfaced an undocumented conflict this spec and `docs/archive/spec-pane-attribution.md` never reconciled — `PaneMetrics` is on-demand, opted in ONLY from the breakdown popover's open/close (`docs/archive/spec-pane-attribution.md`'s gate-resolved "an idle daemon does zero process-table work"), but this spec's Outcome requires the working/idle state to surface in the ALWAYS-VISIBLE tab/status-line indicator, which needs live per-pane CPU independent of whether anyone has that popover open. Resolved (activation model): enable `PaneMetrics` on-demand while the attached session has >=1 non-shell Busy pane (`SessionView::has_busy_pane`) — i.e. "sample when there is something to attribute/classify" — via an app-side reference-counted/OR'd enable (`WorkspaceView`'s `PaneMetricsOptIn`) that merges the classifier's want with the popover's existing want; `ClientMessage::SetPaneMetricsEnabled` is sent only on an actual flip of that OR'd result. This satisfies the working/idle indicator (live CPU exactly when a busy pane exists) AND preserves the pane-attribution spec's zero-idle-cost decision (a truly idle session — no busy panes, no popover open — still enables nothing); an always-on-per-attach model was rejected precisely because it would reverse that cost decision instead of extending it coherently. Implementation: `crates/app/src/activity_classifier.rs` — `CpuWorkClassifier`, a pure per-pane threshold (5% CPU, an unpinned-by-spec, tunable implementation default mirroring `docs/archive/spec-memory-pressure.md`'s threshold precedent) + hysteresis (2 consecutive samples, i.e. one settling window at the daemon's ~2s `PANE_METRICS_INTERVAL` cadence) state machine, defaulting to `WorkState::Working`. `crates/terminal/src/session_view.rs` gained `SessionView::has_busy_pane` (the classifier's opt-in condition) and `SessionView::set_pane_work_state` (applies a classified `WorkState` only to a currently non-shell Busy pane). `WorkspaceView::apply_pane_metrics` feeds each `PaneMetrics` entry's CPU to that pane's own classifier and pushes the result through `SessionView::set_pane_work_state`; classifier state per pane is never reset on its own (a pane that stops being sampled simply keeps its last classified state — the least-surprising option, since a still-busy pane never actually loses its `PaneMetrics` opt-in under this activation model, and a pane that goes free ignores `work_state` entirely regardless).
